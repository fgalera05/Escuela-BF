...
function PasarDeAnio({ alumno, course, pasarDeAnio }) {
  ...
  const handleClickGuardar = async (data) => {
    ...
    axios
      .patch(
        process.env.REACT_APP_URL + "alumnos/pasardeanio/" + alumnoQuePasa,
        {
  ...
  useEffect(() => {
    axios
      .get(process.env.REACT_APP_URL + "cursos/pasar/" + alumno[1]._id, {
        headers: {
          Authorization: "Bearer " + token,
        },
      })
  ...
  return (
    <>
      ...
      <Dialog open={open} onClose={handleClose}>
        <DialogTitle>
          Pasa de año: {alumno[1].alumno.apellido}, {alumno[1].alumno.nombre}{" "}
        </DialogTitle>
        <DialogContent>
          <DialogContentText>
            Elija el curso dónde se inscribe:
          </DialogContentText>
          <Box
            component="form"
            sx={{
          ...
          >
            <div>
              ...
              >
                ...
                {errors.seAnotaEn && (
                  <p role="alert">{errors.seAnotaEn?.message}</p>
                )}
              </TextField>
            </div>
          </Box>
